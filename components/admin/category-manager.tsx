          ...
          {categories.map((c) => (
            <li key={c.id} className="flex items-center gap-2 text-[14px]">
              <span className="min-w-0 flex-1 truncate">
                {c.title}
                <span className="ml-1.5 text-[var(--brand-muted)]">({c.postCount})</span>
              </span>
              {c.postCount === 0 && (
                <button
                  type="button"
      ...
      <form action={formAction}>
        ...
        <div className="flex gap-2">
          <input
            ...
            className="min-w-0 flex-1 rounded-[var(--radius-sm)] border border-[var(--brand-line)] bg-white px-3 py-2 text-[14px] outline-none focus:border-[var(--brand-primary)]"
          />
          <button
            type="submit"
