...
export async function saveQuickReference(
  _prev: CrudState,
  fd: FormData,
): Promise<CrudState> {
  ...
  try {
    ...
    await db.quickReference.create({
...
export async function moveQuickReference(
  id: string,
  direction: "up" | "down",
): Promise<void> {
  if (!(await requireAdmin())) return;
...
export async function togglePublish(id: string): Promise<void> {
  if (!(await requireAdmin())) return;
  const row = await db.quickReference.findUnique({
    where: { id },
    select: { isPublished: true },
  });
  if (!row) return;
  await db.quickReference.update({
    where: { id },
