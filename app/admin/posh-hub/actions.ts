...
const sectionSchema = z.object({
  ...
  anchor: z
    .string()
    .trim()
    .regex(/^[a-z0-9]+(?:-[a-z0-9]+)*$/, 'Anchors are lowercase letters, digits and hyphens')
    .max(96),
...
export async function saveSection(_prev: CrudState, fd: FormData): Promise<CrudState> {
  ...
  try {
    ...
    if (isUniqueViolation(err)) {
      return { status: 'error', message: 'That anchor is already used by another section.' }
    }
...
export async function moveSection(id: string, direction: 'up' | 'down'): Promise<void> {
  if (!(await requireAdmin())) return
  const row = await db.poshSection.findUnique({ where: { id }, select: { group: true } })
  if (!row) return
  ...
  await db.$transaction([
    db.poshSection.update({ where: { id: swap.a.id }, data: { order: swap.a.order } }),
    db.poshSection.update({ where: { id: swap.b.id }, data: { order: swap.b.order } }),
  ])
  bust()
}
