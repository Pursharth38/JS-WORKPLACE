...
export default async function AdminPage() {
  ...
  const [
    ...
    db.enrollment.count(),
    db.payment.aggregate({ where: { status: 'PAID' }, _sum: { amountInPaise: true }, _count: true }),
    db.lead.count(),
    ...
    db.certificate.findMany({
      ...
      select: { certId: true, learnerName: true, issuedAt: true, revokedAt: true },
    }),
  ])
    ...
    <div>
        ...
        <AdminSection title="Recent payments">
          ...
          <p className="mt-3 text-[14px] text-[var(--brand-muted)]">
            REFUNDED payments do <strong>not</strong> auto-revoke access — review them here
            and revoke the enrolment/certificate manually if warranted.
          </p>
        </AdminSection>
        ...
        <AdminSection title="Leads">
          <p className="mb-4 text-[15px] leading-relaxed text-[var(--brand-muted)]">
            Export every lead captured by the demo, checklist, compliance-check and
            newsletter forms, with consent timestamps, as a spreadsheet.
          </p>
          <a
...
function Table({
  ...
  if (rows.length === 0) {
    return (
      <p className="rounded-lg border border-dashed border-[var(--brand-line)] bg-white p-6 text-[15px] text-[var(--brand-muted)]">
        {empty}
      </p>
    )
  }
  ...
  return (
    <div className="overflow-x-auto rounded-lg border border-[var(--brand-line)] bg-white">
      <table className="w-full min-w-[640px] border-collapse text-[14px]">
        ...
        <tbody>
          {rows.map((cells, i) => (
            <tr key={i} className="border-b border-[var(--brand-line)] last:border-0">
              {cells.map((c, j) => (
