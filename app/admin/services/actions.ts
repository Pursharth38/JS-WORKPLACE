...
export async function saveService(_prev: CrudState, fd: FormData): Promise<CrudState> {
  ...
  try {
    ...
    await db.service.create({ data: { ...data, order: (max._max.order ?? 0) + 1 } })
  } catch (err) {
    if (isUniqueViolation(err)) {
      return { status: 'error', message: 'That slug is already in use by another service.' }
    }
...
export async function moveService(id: string, direction: 'up' | 'down'): Promise<void> {
  ...
  await db.$transaction([
    db.service.update({ where: { id: swap.a.id }, data: { order: swap.a.order } }),
    db.service.update({ where: { id: swap.b.id }, data: { order: swap.b.order } }),
  ])
  bust()
}
