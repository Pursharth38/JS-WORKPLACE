...
import { checkbox, jsonField, lines, optStr, reqStr } from '@/lib/admin-content'
...
export async function savePost(_prev: CrudState, fd: FormData): Promise<CrudState> {
  ...
  if (!parsed.success) {
    ...
    return { status: 'error', message: first?.message ?? 'Check the form — title, slug, excerpt and body are required.' }
  }
  ...
  try {
    ...
    if (isUniqueViolation(err)) {
      return { status: 'error', message: 'That slug is already in use by another post.' }
    }
...
export async function saveCategory(_prev: CrudState, fd: FormData): Promise<CrudState> {
  if (!(await requireAdmin())) return { status: 'error', message: 'Not authorised.' }
  ...
  if (!parsed.success) return { status: 'error', message: 'Give the category a name.' }

  try {
    await db.blogCategory.create({ data: parsed.data })
  } catch (err) {
    if (isUniqueViolation(err)) {
      return { status: 'error', message: 'A category with that slug already exists.' }
    }
  ...
  bust()
  return { status: 'success', message: `Category "${parsed.data.title}" created.` }
}
