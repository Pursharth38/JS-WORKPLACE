  ...
  return (
    <div>
      ...
      {user && !user.emailVerified && (
        <div className="mb-8 rounded-lg border border-[var(--brand-accent)] bg-[var(--brand-elevated)] p-5">
          <p className="text-[16px] font-semibold">
            Confirm your email address
          </p>
          <p className="mt-1 text-[15px] leading-relaxed text-[var(--brand-muted)]">
            We still need to confirm your email before we can issue a
      ...
      {withProgress.length === 0 ? (
        ...
        <ul className="grid gap-4">
          {withProgress.map((e) => (
            <li
              key={e.courseId}
              className="rounded-lg border border-[var(--brand-line)] bg-[var(--brand-elevated)] p-6"
            >
              <div className="flex flex-wrap items-start justify-between gap-4">
                <div className="min-w-0">
                  ...
                  <p className="mt-1 text-[14px] text-[var(--brand-muted)]">
                    Enrolled{" "}
                    {e.enrolledAt.toLocaleDateString("en-IN", {
                      day: "numeric",
                      month: "long",
                      year: "numeric",
                    })}
...
function ProgressBar({ percent }: { percent: number }) {
  ...
  return (
    <div className="mt-5">
      ...
      <p className="mt-1.5 text-[14px] text-[var(--brand-muted)]">
        {clamped}% complete
      </p>
    </div>
  );
  ...
  return (
    <div className="rounded-lg border border-dashed border-[var(--brand-line)] bg-[var(--brand-elevated)] p-10 text-center">
      ...
      <p className="mx-auto mt-2 max-w-[46ch] text-[16px] leading-relaxed text-[var(--brand-muted)]">
        Browse the POSH awareness course to see what it covers, or read the
        knowledge hub for free.
      </p>
