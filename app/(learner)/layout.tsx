    ...
    <div className="min-h-screen bg-[var(--brand-sand)]">
      <header className="border-b border-[var(--brand-line)] bg-white">
        <div className="mx-auto flex max-w-5xl flex-wrap items-center gap-x-6 gap-y-3 px-6 py-4">
          <Link
            href="/"
            className="font-[family-name:var(--font-heading)] text-[18px] font-semibold text-[var(--brand-teal)]"
          >
            JS Workplace Wellness
          </Link>

          <nav className="flex items-center gap-5 text-[15px]" aria-label="Learner area">
            <Link href="/dashboard" className="hover:text-[var(--brand-teal)]">
              Dashboard
            </Link>
            <Link href="/dashboard/certificates" className="hover:text-[var(--brand-teal)]">
              Certificates
            </Link>
            <Link href="/dashboard/invoices" className="hover:text-[var(--brand-teal)]">
              Invoices
            </Link>
            {session.role === 'ADMIN' && (
              <Link href="/admin" className="font-medium text-[var(--brand-amber)]">
                Admin
              </Link>
            )}
          </nav>
